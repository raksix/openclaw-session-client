...
function authenticate(nonce: string) {
  ...
  pendingRequests.set(id, {
    resolve: () => { console.log("[Auth] Başarılı!"); authed = true; listSessions(); },
  ...
  // Try webchat client ID
  ws.send(JSON.stringify({
    type: "req", id, method: "connect", params: {
      minProtocol: 3, maxProtocol: 3,
      client: { id: "webchat", displayName: "WebChat Client", version: "1.0.0", platform: "linux", mode: "web" },
