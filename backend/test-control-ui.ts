...
ws.on("message", (data) => {
  ...
  if (msg.event === "connect.challenge") {
    ...
    // Try openclaw-control-ui client id
...
function authenticate(nonce: string) {
  ...
  // Try with openclaw-control-ui client id
  ws.send(JSON.stringify({
    type: "req", id, method: "connect", params: {
      minProtocol: 3, maxProtocol: 3,
      client: { id: "openclaw-control-ui", displayName: "Control UI", version: "1.0.0", platform: "linux", mode: "backend" },
